            ...
            try
            {
                ...
                * */
                ...
                if (isflask)
                {
                    ...
                    FlaskingAlgo fago = new FlaskingAlgo();
                    fago.FirstCompare = firstcompare;
                    Fla_Box[] fbs = fago.Flasking(ant_sort, dic_anttotags_sort, antareamaxc, latp);

                    dic_anttotags_sort.Clear();

                    for (int i = 0; i < fbs.Length; i++)
                    {
                        List<string> ltag = new List<string>();
                        for (int j = 0; j < fbs[i].FBalls.Count; j++)
                            ltag.Add(fbs[i].FBalls[j].Ballid);

                        dic_anttotags_sort.Add(fbs[i].Boixid, ltag);
                    }
                }
                else
                {
